...
- **Tikhonov / ridge** — penalizes magnitude ${\left\lVert \phi \right\rVert ^2}$ (**gauge-dependent**:
...
orders of magnitude in $\lambda_1$, whereas ridge **inverts** the ordering
(`-0.42`) for every $\lambda_1 > 0$. Everything below regenerates the exact numbers
...
- **E4 nilpotency** — $\left\lVert {A^m} \right\rVert_F = 0$ at the predicted index.
...
| Experiment | Result |
|---|---|
| E1 sweep ($\lambda_1=1$, HHD, n=95) | Sobolev Spearman **+0.81** / NDCG@5 **0.999**; ridge **−0.42** / 0.756 |
| E1 across $\lambda_1$ in [1e-3, 10]` | Sobolev Spearman constant at **+0.807**; ridge negative throughout |
| E2 chain range ($\lambda_1=1$) | Sobolev Δφ **0.968**; ridge **0.536** |
| E3 sink discovery | residual ranks all 5 sinks in top **7** of 277; potential ranking worst **108** |
| E4 nilpotency | $\left\lVert {A^m} \right\rVert_F = 0$ at **m = 7** |
...
- **Gauge-stable solve.** Theorem 1 makes the reduced operator `M` SPD *when
  ...
  path runs floor-free from $x_0 = 0$; both keep the gauge mode exact where a

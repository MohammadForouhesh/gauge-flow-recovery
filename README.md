...
validate claims that need no planted field. **Get the data** — download the one
file listed per dataset (Kaggle requires a free login; the Kaggle files arrive
zipped, so unzip first). No preprocessing is needed: the loaders consume the raw
file and build the state space automatically.

| Dataset | File to download | Where |
|---|---|---|
| RetailRocket | `events.csv` | https://www.kaggle.com/datasets/retailrocket/ecommerce-dataset |
| Trivago | `train.csv` | RecSys 2019 Challenge data, recsys2019data.trivago.com (free signup) |
| OTTO | `train.jsonl` | https://www.kaggle.com/competitions/otto-recommender-system/data |

Point each run at the file with the flag shown below (any path works). Each
command writes `results/results_<dataset>.json` and prints a summary:
...
Reuses the same OTTO `train.jsonl` as §4.3 (no extra download):
